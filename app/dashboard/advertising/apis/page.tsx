...
export default function ApisPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editingApi) {
        ...
        const { error } = await supabase.from("apis").update(apiData).eq("id", editingApi.id)
    ...
    try {
      ...
      if (usageData && usageData.length > 0) {
        toast({
          ...
          description: "Esta API está siendo utilizada por uno o más anuncios en servidores",
          variant: "destructive",
        })
        return
      }
  ...
  async function handleToggleActive(id: string, isActive: boolean) {
    try {
      const { error } = await supabase.from("apis").update({ is_active: !isActive }).eq("id", id)
      ...
      toast({
        ...
        description: `La API ha sido ${!isActive ? "activada" : "desactivada"} correctamente`,
      })
    } catch (err: any) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Administra las APIs para la integración con plataformas publicitarias</p>
        </div>
                  ...
                  <TableRow key={api.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        <Switch
                          ...
                          onCheckedChange={() => handleToggleActive(api.id, api.is_active)}
                        />
                        <Badge variant={api.is_active ? "outline" : "secondary"}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end space-x-2">
                        <Button variant="outline" size="icon" onClick={() => handleEditApi(api)}>
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editingApi ? "Editar API" : "Crear Nueva API"}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4 py-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <Label htmlFor="isActive">API Activa</Label>
            </div>
            ...
            <div className="flex justify-end space-x-2 pt-4">
              ...
              <Button type="submit">{editingApi ? "Actualizar" : "Crear"}</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
